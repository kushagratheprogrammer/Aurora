...
<input type='submit' class='btn btn-primary' name='clarreply' value='Reply / Change Reply'/>
        ...
        if (isset($_SESSION['loggedin']) && $_SESSION['team']['status'] == "Admin") {
            $query = "select distinct(pgroup) from problems where contest='practice' order by pid desc";
        } else {
            $query = "select distinct(pgroup) from problems where status != 'Deleted' and contest='practice' order by pid desc";
        }
        $result = DB::findAllFromQuery($query);
        $group = Array();
        foreach ($result as $row) {
            array_push($group, $row['pgroup']);
        }
